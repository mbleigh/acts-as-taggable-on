...
describe ActsAsTaggableOn::Taggable::Dirty do
  [
    [TaggableModel, OrderedTaggableModel],
    [TaggableNamespacedModel, OrderedTaggableNamespacedModel]
  ].each do |m|

    context 'with un-contexted tags' do
      before(:each) do
        @taggable = m[0].create(tag_list: 'awesome, epic')
      end
      ...
      context 'when tag_list changed' do
        ...
        context 'without order' do
          it 'should not mark attribute if order change ' do
            taggable = m[0].create(name: 'Dirty Harry', tag_list: %w(d c b a))
        ...
        context 'with order' do
          it 'should mark attribute if order change' do
            taggable = m[1].create(name: 'Clean Harry', tag_list: 'd,c,b,a')
            taggable.save
            taggable.tag_list =  %w(a b c d)
            expect(taggable.tag_list_changed?).to be_truthy
          end
    ...
    context 'with context tags' do
      before(:each) do
        @taggable = m[0].create('language_list' => 'awesome, epic')
      end
