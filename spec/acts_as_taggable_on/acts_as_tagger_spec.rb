...
describe 'acts_as_tagger' do
  ...
  describe '#tag' do
    context 'when called with a non-existent tag context' do
      def set_tags(m)
        @tagger = User.new
        @taggable = m.new(name: 'Richard Prior')
      end
    ...
    describe "when called by multiple taggers" do
      def set_tags(m)
        @user_x = m[0].create(name: 'User X')
        @user_y = m[0].create(name: 'User Y')
        @taggable = m[1].create(name: 'acts_as_taggable_on', tag_list: 'plugin')
      ...
      [ [User, TaggableModel], [NamespacedUser, TaggableNamespacedModel] ].each do |m|
