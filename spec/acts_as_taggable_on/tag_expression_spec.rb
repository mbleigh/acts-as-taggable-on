...
describe "Tag Expressions" do
  ...
  it "should allow the use of whitespace to separate tags and operators" do
    ...
    TaggableModel.tagged_with("c++ + python", :expression => { :use_whitespace => true }).to_a.length.should == 2
    TaggableModel.tagged_with("c++ ++++++++++ python", :expression => { :use_whitespace => true }).to_a.length.should == 2
  ...
  it "should handle repeated operators" do
    ...
    TaggableModel.tagged_with("c\\+\\+++++python", :expression => { :use_whitespace => false }).to_a.length.should == 2
    TaggableModel.tagged_with("c++ ++++++++++ python", :expression => {:use_whitespace => true }).to_a.length.should == 2
  end
  ...
  it "should allow union to act as an intersection using default_chaining option" do
    bob = TaggableModel.create(:name => "Bob", :tag_list => "sad, lazy, happy, grumpy")
    TaggableModel.create(:name => "Frank", :tag_list => "happy, lazy, strong")
    TaggableModel.create(:name => 'Steve', :tag_list => "happy, grumpy, lazy")
    TaggableModel.create(:name => 'Steve+Molly', :tag_list => "sad, strong, lazy, happy")

    default_chain = TaggableModel.tagged_with("sad+lazy+happy+grumpy", :expression => {:default_chaining => true}).to_a
    intersection_chain = TaggableModel.tagged_with("sad&lazy&happy&grumpy", :expression => {:default_chaining => false}).to_a

    default_chain.should == [bob]
    intersection_chain.should == [bob]
  end
end
