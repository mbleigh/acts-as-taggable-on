...
describe 'Taggable' do
  ...
  it 'should be able to select taggables by subset of tags using ActiveRelation methods' do
    ...
    @found_taggables_by_tag = TaggableModel.joins(:tags).where(acts_tags: {name: ['bob']})
    @found_taggables_by_skill = TaggableModel.joins(:skills).where(acts_tags: {name: ['ruby']})
  ...
  it 'should be able to get all tag counts on model as whole' do
    ...
    expect(TaggableModel.all_tag_counts(order: 'acts_tags.id').first.count).to eq(3) # ruby
  end
  ...
  it 'should be able to get all tags on model as whole' do
    ...
    expect(TaggableModel.all_tags(order: 'acts_tags.id').first.name).to eq('ruby')
  end
  ...
  it 'should be able to get scoped tag counts' do
    ...
    expect(TaggableModel.tagged_with('ruby').tag_counts(order: 'acts_tags.id').first.count).to eq(2) # ruby
  ...
  it 'should be able to get all scoped tag counts' do
    ...
    expect(TaggableModel.tagged_with('ruby').all_tag_counts(order: 'acts_tags.id').first.count).to eq(3) # ruby
  end
  ...
  it 'should be able to get all scoped tags' do
    ...
    expect(TaggableModel.tagged_with('ruby').all_tags(order: 'acts_tags.id').first.name).to eq('ruby')
  end
  ...
  describe 'grouped_column_names_for method' do
    it 'should return all column names joined for Tag GROUP clause' do
      ...
      .to eq('acts_tags.id, acts_tags.name, acts_tags.taggings_count, acts_tags.type')
    end
