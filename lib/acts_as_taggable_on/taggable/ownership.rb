module ActsAsTaggableOn::Taggable
  module Ownership
    ...
    def owner_tags_on(owner, context)
      if owner.nil?
        scope = base_tags.where([%(#{self.namespaced(:tagging).table_name}.context = ?), context.to_s])
      else
        scope = base_tags.where([%(#{self.namespaced(:tagging).table_name}.context = ? AND
                                    #{self.namespaced(:tagging).table_name}.tagger_id = ? AND
                                    #{self.namespaced(:tagging).table_name}.tagger_type = ?), context.to_s, owner.id, owner.class.base_class.to_s])
      end

      # when preserving tag order, return tags in created order
      # if we added the order to the association this would always apply
      if self.class.preserve_tag_order?
        scope.order("#{self.namespaced(:tagging).table_name}.id")
      else
        scope
      end
    end
    ...
    def save_owned_tags
      tagging_contexts.each do |context|
        cached_owned_tag_list_on(context).each do |owner, tag_list|
          ...
          self.namespaced(:tagging).destroy_all(taggable_id: id, taggable_type: self.class.base_class.to_s,
